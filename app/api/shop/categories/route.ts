...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const validatedData = ShopCategorySchema.omit({ createdAt: true, updatedAt: true }).parse(body);

    const category = new ShopCategory(validatedData);
    await category.save();

    return NextResponse.json({ message: 'Category created', id: category._id }, { status: 201 });
  } catch (error) {
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const itemCount = await ShopItem.countDocuments({ categoryId: validatedData.id });
    if (itemCount > 0) {
